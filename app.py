...
def fetch_and_rename(filename):
    ...
    if df.empty and GITHUB_USER and GITHUB_REPO:
        try:
            url = f"[https://raw.githubusercontent.com/](https://raw.githubusercontent.com/){GITHUB_USER}/{GITHUB_REPO}/main/{filename}"
            df = pd.read_csv(url)
        except: pass
...
# 🌟 新增：純粹且無懈可擊的 Markdown 選項產生器 (利用 CSS 攔截並渲染)
def get_strat_label(key, base_name):
    ...
    if val is not None:
        sign = "+" if val > 0 else ""
        if val > 0:
            # 正報酬：使用刪除線 ~~ (CSS 會將其攔截並渲染成紅色徽章)
            return f"**{key}. {base_name}**~~AVG: {sign}{val:.2f}%~~"
        elif val < 0:
            # 負報酬：使用斜體 * (CSS 會將其攔截並渲染成綠色徽章)
            return f"**{key}. {base_name}***AVG: {sign}{val:.2f}%*"
        else:
            # 零報酬：使用程式碼 ` (CSS 會將其攔截並渲染成灰色徽章)
            return f"**{key}. {base_name}**`AVG: {sign}{val:.2f}%`"
