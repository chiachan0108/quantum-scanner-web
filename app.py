...
.result-summary:hover, .return-summary:hover {
  background: rgba(0, 242, 255, 0.12);
  border-color: rgba(0, 242, 255, 0.6);
  box-shadow: 0 0 15px rgba(0, 242, 255, 0.15);
}

/* 修正內部文字，實現左右、上下幾何對稱平衡 */
.prefix, .suffix {
  display: inline-flex;
  align-items: center;
  font-weight: 800;
...
.prefix {
  width: 80px; /* 給前綴一個固定寬度 */
  justify-content: flex-end; /* 右對齊確保冒號對齊 */
}
.suffix {
  width: 50px; /* 給後綴一個固定寬度 */
  justify-content: flex-start; /* 左對齊 */
}

/* 應用上下幾何居中微調 */
.result-num, .return-val-up, .return-val-down, .return-val-zero, .prefix, .suffix, .value {
  transform: translateY(1px); /* 向上向下幾何居中微調 */
}
...
if not st.session_state['scan_completed']:
    ...
    strategy_choice = st.radio("量化策略模組", ["A. 營收趨勢增長型", "B. 股價強勢動能型", "C. 營收股價雙能型", "D. 法人籌碼吃貨型", "E. 市場區間共振型", "F. 左側超跌優質型", "S. 趨勢轉折延伸型"], label_visibility="collapsed")
    ...
    # 將 '%' 分離，放在 suffix 中，並微調 prefix 的寬度確保對齊冒號。
    avg_ret_html = f'''
    ...
    <div class="summary-box-group">
      <div class="result-summary">
        <span class="prefix result-text">共篩選出 :</span>
        <span class="result-num">{len(df)}</span>
        <span class="suffix result-text">檔標的</span>
      </div>
      <div class="return-summary">
        <span class="prefix result-text">平均漲幅 :</span>
        <span class="value {ret_class}">{ret_sign}{avg_ret:.2f}</span>
        <span class="suffix result-text">%</span>
      </div>
    </div>
    '''
