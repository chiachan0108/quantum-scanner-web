...
# 🌟 定義所有策略的邏輯說明文字 (搬移至分頁上方供呼叫，策略 M 擴充字數對齊版面)
logic_dict = {
    ...
    "M.": '<div class="logic-grid"><div class="logic-item"><div class="logic-header"><div class="logic-tag-row"><span class="logic-index-tag">01</span><span class="logic-label-en">SCOPE</span></div><div class="logic-label-zh">選股範圍</div></div><div class="logic-desc">鎖定台灣<span class="highlight">全體上市櫃普通股</span>標的，排除 ETF、ETN 與異常狀態之個股。</div></div><div class="logic-item"><div class="logic-header"><div class="logic-tag-row"><span class="logic-index-tag">02</span><span class="logic-label-en">LIQUIDITY</span></div><div class="logic-label-zh">流動性門檻</div></div><div class="logic-desc">近20日平均日成交量需大於<span class="highlight">500張</span>，確保標的之市場交易活躍度與進出流動性無虞。</div></div><div class="logic-item"><div class="logic-header"><div class="logic-tag-row"><span class="logic-index-tag">03</span><span class="logic-label-en">HISTORY</span></div><div class="logic-label-zh">歷史基期</div></div><div class="logic-desc">具備至少 <span class="highlight">24 個月</span>以上的歷史營收數據，藉由足夠的歷史資料樣本，避免新上市股票的短期失真。</div></div><div class="logic-item"><div class="logic-header"><div class="logic-tag-row"><span class="logic-index-tag">04</span><span class="logic-label-en">HIGH-WATER</span></div><div class="logic-label-zh">動態創高</div></div><div class="logic-desc">統計過去 12 個月中，單月營收突破「從上市以來至當月為止之歷史紀錄」的次數高達 <span class="highlight">5 次以上</span>，確認極強主升段爆發動能。</div></div></div>',
...
if not st.session_state['scan_completed']:
    ...
    with st.expander("◈ 個股查詢 (輸入代號或名稱)", expanded=False):
        ...
        if search_query:
            # 🌟 新增載入 J、K、L、M 策略
            s_a, s_b, s_d, s_e, s_f, s_g, s_h, s_j, s_k, s_l, s_m = map(fetch_and_rename, [
            ...
            # 策略 R 判定邏輯 (A, B, D, E, F, G, H, J, K, L, M 取 3)
            base_strats = {"A. 營收趨勢增長型", "B. 股價強勢動能型", "D. 法人籌碼吃貨型", "E. 市場區間共振型", "F. 左側超跌優質型", "G. 中長周期轉折型", "H. 財報三率三升型", "J. 指標強勢共振型", "K. 跨週期多矩陣型", "L. 股本法人鎖碼型", "M. 營收創高精選型"}
    ...
    with t_chip:
        strat_chip = st.radio("籌碼面區", ["D. 法人籌碼吃貨型", "E. 市場區間共振型", "F. 左側超跌優質型", "L. 股本法人鎖碼型"], label_visibility="collapsed")
        st.markdown("<div class='section-header-container' style='margin-top: 15px;'><div class='section-accent'></div><div class='section-header-text'><span class='section-label-en'>SYSTEM ARCHITECTURE</span><span class='section-label-zh'>策略核心邏輯</span></div><div class='section-line'></div></div>", unsafe_allow_html=True)
