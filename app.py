...
if not st.session_state['scan_completed']:
    ...
    with st.expander("◈ 個股查詢 (輸入代號或名稱)", expanded=False):
        ...
        if search_query:
            ...
            base_strats = {"A. 營收趨勢增長型", "B. 股價強勢動能型", "D. 法人籌碼吃貨型", "E. 市場區間共振型", "F. 左側超跌優質型", "G. 中長周期轉折型", "H. 財報三率三升型", "J. 極長線趨勢鎖碼型", "K. 跨週期矩陣爆發型", "L. 股本法人鎖碼型"}
    ...
    with t_fund:
        st.radio("基本面區", ["A. 營收趨勢增長型", "H. 財報三率三升型", "I. 營收財報雙能型"], 
                 label_visibility="collapsed", key="rad_fund", on_change=update_strat, args=("rad_fund",))
    with t_chip:
        st.radio("籌碼面區", ["D. 法人籌碼吃貨型", "E. 市場區間共振型", "F. 左側超跌優質型", "L. 股本法人鎖碼型"], 
    ...
    # 將最後選擇的策略賦值給主程式變數
    strategy_choice = st.session_state['active_strat']
