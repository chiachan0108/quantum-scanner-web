...
if not st.session_state['scan_completed']:
    ...
    with st.expander("◈ 個股查詢 (輸入代號或名稱)", expanded=False):
        ...
        if search_query:
            # 🌟 新增載入 J、K、L 策略
            s_a, s_b, s_d, s_e, s_f, s_g, s_h, s_j, s_k, s_l = map(fetch_and_rename, [
            ...
            # 策略 R 判定邏輯 (A, B, D, E, F, G, H, J, K, L 取 3)
            base_strats = {"A. 營收趨勢增長型", "B. 股價強勢動能型", "D. 法人籌碼吃貨型", "E. 市場區間共振型", "F. 左側超跌優質型", "G. 中長周期轉折型", "H. 財報三率三升型", "J. 指標強勢共振型", "K. 跨週期多矩陣型", "L. 股本法人鎖碼型"}
    ...
    with t_fund:
        strat_fund = st.radio("基本面區", ["A. 營收趨勢增長型", "H. 財報三率三升型", "I. 營收財報雙能型"], label_visibility="collapsed")
    ...
    if run_clicked:
        ...
        try:
            # 載入所有策略資料
            df1, df2, df_d, df_e, df_squat, df_g, df_h, df_j, df_k, df_l = map(fetch_and_rename, [
                "strategy_a_result.csv", "strategy_b_result.csv", "strategy_d_result.csv", 
                "strategy_e_result.csv", "strategy_f_result.csv", "strategy_g_result.csv", 
                "strategy_h_result.csv", "strategy_j_result.csv", "strategy_k_result.csv",
                "strategy_l_result.csv"
            ...
            if "A." in strategy_choice: df_f = df1
                ...
                target_sids = [sid for sid, tags in id_map.items() if len(tags.intersection({"A", "B", "D", "E", "F", "G", "H", "J", "K", "L"})) >= 3]
                dfs_to_concat = [d for d in [df1, df2, df_d, df_e, df_squat, df_g, df_h, df_j, df_k, df_l] if not d.empty]
                ...
                for sid, tags in id_map.items(): 
                    ...
                    if len(tags.intersection({"A", "B", "D", "E", "F", "G", "H", "J", "K", "L"})) >= 3: tags.add("R")
