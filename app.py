...
# [CSS 樣式終極修正版] - 滿足所有像素級細節與發光質感，徹底消滅所有點點
...
/* 🔥 全域 Radio 魔法 - 徹底拔除原生點點，鎖定像素細節，選取後外框發光 */
...
/* 取消我們之前自製的圓點，確保乾淨 */
[data-testid="stRadio"] div[role="radiogroup"] label div[data-testid="stMarkdownContainer"] > p > strong::before {
...
[data-testid="stRadio"] div[role="radiogroup"] label div[data-testid="stMarkdownContainer"] > p del {
...
/* 攔截斜體(em) - 負報酬綠色 Badge */
[data-testid="stRadio"] div[role="radiogroup"] label div[data-testid="stMarkdownContainer"] > p em {
    font-style: normal !important;
    color: #00ff33 !important;
    background: rgba(0, 255, 51, 0.08) !important;
    border: 1px solid rgba(0, 255, 51, 0.25) !important;
    padding: 4px 14px !important;
    border-radius: 20px !important;
    font-size: 0.85rem !important;
    font-family: 'JetBrains Mono', monospace !important;
...
[data-testid="stRadio"] div[role="radiogroup"] label div[data-testid="stMarkdownContainer"] > p code {
...
# 🌟 新增：最安全、無原先 HTML 的 Markdown 文字產生器 (搭配 CSS 徹底防彈)
...
if not st.session_state['scan_completed']:
    ...
    if (run_fund or run_chip or run_tech or run_multi):
        ...
        for k, v in strat_map.items():
            # In Streamlit button state is True when clicked.
        ...
        if is_t_valid:
            ...
            try:
                ...
                if active_key == "A": df_f = df1
                ...
                elif active_key == "C": df_f = df1[df1['代號'].isin(set(df1['代號']).intersection(set(df2['代號'])))].copy() if not df1.empty and not df2.empty else pd.DataFrame()
                ...
                elif active_key == "I": df_f = df1[df1['代號'].isin(set(df1['代號']).intersection(set(df_h['代號'])))].copy() if not df1.empty and not df_h.empty else pd.DataFrame()
                    ...
                    df_f = df_r.copy()
                    if not df_f.empty: df_f['名稱'] = df_f.apply(lambda r: f"{r['名稱']} ({','.join(sorted(list(id_map.get(str(r['代號']), set()))))})" if id_map.get(str(r['代號'])) else r['名稱'], axis=1)
                elif active_key == "S":
                    df_f = df_s.copy()
                    if not df_f.empty: df_f['名稱'] = df_f.apply(lambda r: f"{r['名稱']} ({','.join(sorted(list(id_map.get(str(r['代號']), set()))))})" if id_map.get(str(r['代號'])) else r['名稱'], axis=1)
                elif active_key == "T":
                    ...
                    for k in selected_keys:
                        df_map = {'A': df1, 'B': df2, 'D': df_d, 'E': df_e, 'F': df_squat, 'G': df_g, 'H': df_h, 'J': df_j, 'K': df_k, 'L': df_l, 'M': df_m, 'N': df_n, 'O': df_o, 'R': df_r, 'S': df_s}
                        curr_df = df_map.get(k, pd.DataFrame())
            ...
            if not df_f.empty:
                ...
                st.session_state.update({'temp_df': df_f, 'selected_strategy': active_key, 'scan_completed': True})
    ...
    clean_strategy_title = f"STRATEGY {active_key}. {name_map.get(active_key, '')}"
