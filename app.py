...
[data-testid="stRadio"] div[role="radiogroup"] label {
    ...
    padding: 24px 20px !important; /* 🌟 確保豐滿的呼吸感 */
...
def fetch_and_rename(filename):
    ...
    rename_map = {"股價代號": "代號", "公司名稱": "名稱", "產業別": "產業", "當日漲幅(%)": "漲幅(%)", "季乖離": "季乖離(%)", "年乖離": "年乖離(%)", "月營收MoM(%)": "月營收MoM(%)", "月營收YoY(%)": "月營收YoY(%)", "今年以來累積營收YoY(%)": "今年營收YoY(%)", "近20日法人買賣超(張數)": "20日法人買賣超(張)"}
    if "代號" in df.columns: 
    ...
    with st.expander("輸入代號或名稱", expanded=False):
        ...
        if submit_search and search_query:
            try:
                ...
                if "A. 營收趨勢增長型" in hit_strategies and "B. 股價強勢動能型" in hit_strategies: hit_strategies.append("C. 營收股價雙能型")
                if "A. 營收趨勢增長型" in hit_strategies and "H. 財報三率三升型" in hit_strategies: hit_strategies.append("I. 營收財報雙能型")
                if hit_strategies and match_info["price"] > match_info["pivot"] and match_info["pivot"] > 0: hit_strategies.append("S. 趨勢轉折延伸型")
                ...
                if len([s for s in hit_strategies if s in base_strats]) >= 3: hit_strategies.append("R. 複式策略交集型")
                hit_strategies.sort()
...
if not st.session_state['scan_completed']:
    ...
    with t_multi:
        ...
        if active_multi_key == "T":
            ...
            # 🌟 核心修正 1：當非選取 T 時，顯示選取策略的邏輯說明 (例如 C 或 R)
            st.markdown("<div class='section-header-container' style='margin-top: 15px;'><div class='section-accent'></div><div class='section-header-text'><span class='section-label-en'>SYSTEM ARCHITECTURE</span><span class='section-label-zh'>策略核心邏輯</span></div><div class='section-line'></div></div>", unsafe_allow_html=True)
    ...
    with st.expander("量子篩選結果數據", expanded=True):
        ...
        elif "代號" in disp_df.columns: disp_df = disp_df.set_index("代號")
        ...
        for c in numeric_cols:
            if any(x in c for x in ["法人", "買超", "張", "次數"]): format_dict[c] = "{:,.0f}"
        
        # 🌟 核心修正 2：崩潰版面的固定列寬鎖定 config 🌟
        # 為核心欄位設定固定像素寬度，強制系統不得擠壓，並配合 CSS 強制置中對齊
        col_config = {
        ...
        # 使用 style 來強制文字置中，配合 config 的固定寬度，根絕錯位
